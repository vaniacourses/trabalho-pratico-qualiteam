...
public interface ParcelaRepository extends JpaRepository<Parcela, Long> {
	...
	@Modifying
	@Query(value = """
            insert into parcela (valor_total, valor_desconto, valor_acrescimo, valor_recebido, valor_restante, receber_codigo, quitado, sequencia,\
            data_cadastro, data_vencimento) values (:valor_total, :valor_desconto, :valor_acrescimo, :valor_recebido, :valor_restante, \
            :receber_codigo, :quitado, :sequencia, :data_cadastro, :data_vencimento)\
            """, nativeQuery = true)
	...
	@Modifying
	@Query(value = """
            update parcela set valor_desconto = :vldesconto, valor_acrescimo = :vlacrescimo, valor_recebido = :vlrecebido, \
            valor_restante = :vlrestante, quitado = :quitado, data_pagamento = :data_pagamento where codigo = :codigo\
            """, nativeQuery = true)
	...
	@Query(value = """
            select * from parcela p, receber r, pessoa pes where r.codigo = p.receber_codigo and  pes.codigo = r.pessoa_codigo \
            and pes.codigo = :codigo and p.quitado = :quitado order by p.codigo\
            """, nativeQuery = true)
	public List<Parcela> buscaReceberDaPessoaCodigo(@Param("codigo") Long codpessoa, @Param("quitado") Boolean quitado);
	
	@Query(value = """
            select coalesce(format(sum(p.valor_restante), 2, 'de_DE'), format(0, 2, 'de_DE')) from parcela p, receber r, pessoa pes where r.codigo = p.receber_codigo and  pes.codigo = r.pessoa_codigo \
            and pes.codigo = :codigo and p.quitado = :quitado order by p.codigo\
            """, nativeQuery = true)
