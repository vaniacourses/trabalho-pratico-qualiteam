...
public interface ReceberRepository extends JpaRepository<Receber, Long> {
	...
	@Modifying
	@Query(value = """
            insert into receber (observacao, valor_total, valor_recebido, valor_desconto, valor_acrescimo, valor_restante, quitado, sequencia, \
	...
	@Modifying
	@Query(value = """
            update receber set valor_desconto = :valor_desconto, valor_acrescimo = :valor_acrescimo, valor_recebido = :valor_recebido, \
            valor_restante = :valor_restante, quitado = :quitado, data_finalizado = :data_finalizado where codigo = :codigo\
            """, nativeQuery = true)
	...
	@Query(value = """
            select coalesce(format(sum(p.valor_restante), 2, 'de_DE'), '0,00') from receber r, parcela p where p.receber_codigo = r.codigo \
            and p.quitado = 0\
            """, nativeQuery = true)
