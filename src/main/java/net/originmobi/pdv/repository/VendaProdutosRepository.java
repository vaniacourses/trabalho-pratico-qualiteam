...
public interface VendaProdutosRepository extends JpaRepository<VendaProduto, Long> {
	 ...
	 */
	@Query(value = """
            select p.codigo as cod_prod, p.descricao, p.valor_venda, vp.codigo, p.balanca, vp.valor_balanca from produto p, venda_produtos vp \
            where p.codigo = vp.produto_codigo and vp.venda_codigo = ?1\
            """, nativeQuery = true)
	...
	@Query(value = """
            select produto_codigo, count(produto_codigo) from venda_produtos where venda_codigo = ?1 \
