...
public interface NotaFiscalItemRepository extends JpaRepository<NotaFiscalItem, Long> {
	...
	@Query(value = """
            select nfi.cod_prod, prod.descricao, prod.ncm, nfii.cst, nfi.cfop, prod.unidade, nfi.qtd qtd, format(prod.valor_venda, 2, 'de_DE'), \
            ...
            and nfii.codigo = nfi.imposto_codigo and nfi.nota_fiscal_codigo = ?1\
            """, nativeQuery = true)
